...
SimAeroConfig(tCar *car)
{
    ...
    car->aero.SCx2 = (float) (0.5f * AIR_DENSITY * Cx * FrntArea);
...
SimAeroUpdate(tCar *car, tSituation *s)
{
    ...
    if (airSpeed > 10.0f) {
	float x = car->DynGC.pos.x;
	float y = car->DynGC.pos.y;
	//	float x = car->DynGC.pos.x + cos(yaw)*wing->staticPos.x;
	//	float y = car->DynGC.pos.y + sin(yaw)*wing->staticPos.x;
	float yaw = car->DynGC.pos.az;
	float spdang = atan2(car->DynGCg.vel.y, car->DynGCg.vel.x);
	for (i = 0; i < s->_ncars; i++) {
	    ...
	    float otherYaw = otherCar->DynGC.pos.az;
	    float tmpsdpang = spdang - atan2(y - otherCar->DynGC.pos.y, x - otherCar->DynGC.pos.x);
	    FLOAT_NORM_PI_PI(tmpsdpang);
	    float dyaw = yaw - otherYaw;
		...
		if (fabs(tmpsdpang) > 2.9671) {	    /* 10 degrees */
            ...
            float factor = (float)((fabs(tmpsdpang)-2.9671)/(M_PI-2.9671));

		    tmpas = (float) (1.0 - factor * exp(- 2.0 * DIST(x, y, otherCar->DynGC.pos.x, otherCar->DynGC.pos.y)/(otherCar->aero.Cd * otherCar->DynGC.vel.x)));
		    airSpeed = airSpeed * tmpas;
		} else if (fabs(tmpsdpang) < 0.1396f) {	    /* 8 degrees */
                    float factor = 0.5f * (0.1396f-fabs(tmpsdpang))/(0.1396f);
		    /* before another car - breaks down rear eddies, reduces only drag*/
		    tmpas = (float) (1.0f - factor * exp(- 8.0 * DIST(x, y, otherCar->DynGC.pos.x, otherCar->DynGC.pos.y) / (car->aero.Cd * car->DynGC.vel.x)));
		    dragK = dragK * tmpas;
		}
	    }
	}
    ...
    float dmg_coef = (float)((float)car->dammage / 10000.0);

    car->aero.drag = (float)(-SIGN(car->DynGC.vel.x) * car->aero.SCx2 * v2 * (1.0 + dmg_coef) * dragK * dragK);
    ...
    v2 = car->DynGC.vel.y;
    car->aero.lateral_drag = (float)(-SIGN(v2)*v2*v2*0.7);
    ...
    v2 = car->DynGC.vel.z;
    car->aero.vertical_drag = (float)(-SIGN(v2)*v2*v2*1.5);
...
void SimAeroDamage(tCar *car, sgVec3 poc, float F)
{
    tAero* aero = &car->aero;
    float dmg = (float) (F*0.0001);

    aero->rot_front[0] += (float)(dmg*(urandom()-.5));
    aero->rot_front[1] += (float)(dmg*(urandom()-.5));
    aero->rot_front[2] += (float)(dmg*(urandom()-.5));
    if (sgLengthVec3(car->aero.rot_front) > 1.0) {
        sgNormaliseVec3 (car->aero.rot_front);
    }
    aero->rot_lateral[0] += (float)(dmg*(urandom()-.5));
    aero->rot_lateral[1] += (float)(dmg*(urandom()-.5));
    aero->rot_lateral[2] += (float)(dmg*(urandom()-.5));
    if (sgLengthVec3(car->aero.rot_lateral) > 1.0) {
        sgNormaliseVec3 (car->aero.rot_lateral);
    }
    aero->rot_vertical[0] += (float)(dmg*(urandom()-.5));
    aero->rot_vertical[1] += (float)(dmg*(urandom()-.5));
    aero->rot_vertical[2] += (float)(dmg*(urandom()-.5));
...
SimWingConfig(tCar *car, int index)
{
    ...
    case SIMPLE:
        wing->Kx = (float)(-AIR_DENSITY * area); ///< \bug: there should be a 1/2 here.
        //wing->Kz = 4.0 * wing->Kx;
        wing->Kz = car->options->aero_factor * wing->Kx;
        break;
    case PLANAR:
        wing->Kx = (float)(-AIR_DENSITY * area * 16.0f);
        wing->Kz = wing->Kx;
        break;
    case OPTIMAL:
...
SimWingUpdate(tCar *car, int index, tSituation* s)
{
    ...
    float aoa = atan2(car->DynGC.vel.z, car->DynGC.vel.x); //+ car->DynGC.pos.ay;
    ...
    float airSpeed = car->DynGC.vel.x;
	...
	for (i = 0; i < s->_ncars; i++) {
		...
		if (fabs(tmpsdpang) > 2.9671) {	    /* 10 degrees */
		    /* behind another car - reduce overall airflow */
            float factor = (float)((fabs(tmpsdpang)-2.9671)/(M_PI-2.9671));
		    tmpas = (float)(1.0 - factor*exp(- 2.0 * DIST(x, y, otherCar->DynGC.pos.x, otherCar->DynGC.pos.y) /
    ...
    if (1) {
        ...
        float alpha = 0.0f;
        float vt2b = vt2 * (alpha+(1-alpha)*i_flow);
        vt2b = vt2b * vt2b;
        float hm = (float)(1.5 * (car->wheel[0].rideHeight + car->wheel[1].rideHeight + car->wheel[2].rideHeight + car->wheel[3].rideHeight));
        hm = hm*hm;
        hm = hm*hm;
        hm = (float)(1.0 + exp(-3.0*hm));
    ...
    float cosaoa = cos(aoa);


    if (car->DynGC.vel.x > 0.0f) {
        switch (car->options->aeroflow_model) {
        case SIMPLE:
            wing->forces.x = wing->Kx * vt2 * (1.0f + (float)car->dammage / 10000.0f) * sinaoa;
            wing->forces.z = wing->Kz * vt2 * sinaoa;
            break;
        case PLANAR:
            wing->forces.x = wing->Kx * vt2 * (1.0f + (float)car->dammage / 10000.0f) * sinaoa * sinaoa * sinaoa;
            wing->forces.z = wing->Kz * vt2 * sinaoa * sinaoa * cosaoa;
            break;
        case OPTIMAL:
            wing->forces.x = wing->Kx * vt2 * (1.0f + (float)car->dammage / 10000.0f) * (1.0f - cosaoa);
            wing->forces.z = wing->Kx * vt2 * (1.0f + (float)car->dammage / 10000.0f) * sinaoa;
            break;
	default:
            fprintf (stderr, "Unimplemented option %d for aeroflow model\n", car->options->aeroflow_model);
        }
...
float PartialFlowRectangle(float theta, float psi)
{
    if (psi>0)
        return 1.0;
    if (fabs(psi)>fabs(2.0*theta))
        return 0.0;
    return (float)((1.0-(1.0-sin(psi)/sin(2.0*theta))));
}
 
float PartialFlowSmooth(float theta, float psi)
{
    if (psi>0)
        return 1.0;
    if (fabs(psi)>fabs(2.0*theta))
        return 0.0;
    return (float)((0.5*(1.0+tanh((theta-psi)/(fabs(1.0-(psi/theta))-1.0)))));
}
 
float PartialFlowSphere(float theta, float psi)
{
    if (psi>0)
        return 1.0;
    if (fabs(psi)>fabs(2.0*theta))
        return 0.0;
    return (float)((0.5*(1.0+sin((theta-psi)*M_PI/(2.0*theta)))));
}
...
float Max_SCl_given_Cd (float Cd, float A)
{
    ...
    return (float)(A * Cl * AIR_DENSITY / 2.0f);
}
...
float MaximumLiftGivenDrag (float drag, float A)
{
    ...
    float Cd = (float)((drag / A) * 2.0f / AIR_DENSITY);
